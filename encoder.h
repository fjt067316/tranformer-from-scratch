...
public:
    ...
    Attention* fcl = new Attention(50);


    Encoder()
    {}

    vector<vector<double>>  forwards(string input) {
    ...
    // https://glassboxmedicine.com/2019/08/15/the-transformer-attention-is-all-you-need/
        ...
        vector<vector<double>> feed_forward_out = fcl->feed_forward(sub_l1);
        vector<vector<double>> sub_l2 = fcl->add_and_norm(feed_forward_out);
    // #####################    ENCODER FINISHED    #################
    ...
    vector<vector<double>> backwards(vector<vector<double>> dLdZ){

        vector<vector<double>> dLdZ_2d = fcl->update_norm(dLdZ);
        fcl->feed_backwards(dLdZ_2d);
        dLdZ_2d = attn->update_norm(dLdZ_2d);
        attn->attn_backwards(dLdZ_2d);
