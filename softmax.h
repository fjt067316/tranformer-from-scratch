...
public:
    ...
    vector<vector<double>> forward(vector<vector<double>> input) { // 2D input
        ...
        for (int i = 0; i < input.size(); i++) {
            ...
            for (int j = 0; j < input[0].size(); j++) {
                output[i][j] = exp(input[i][j]-max) / sum;
            }
        }
    ...
    vector<vector<double>> backwards(vector<vector<double>> dLdZ){ // cross_entropy dLdZ = -1/p
        ...
        for(int i=0; i < last_input.size(); i++){
            for(int j=0; j < last_input[0].size(); j++){
                dout_dt[i][j] = -dLdZ_exp[label_i][label_j]*dLdZ_exp[i][j] / (sum_exp*sum_exp);
            }
        }

        dout_dt[label_i][label_j] = dLdZ_exp[label_i][label_j]*(sum_exp-dLdZ_exp[label_i][label_j])/(sum_exp*sum_exp);
        
        for(int i=0; i < last_input.size(); i++){
            for(int j=0; j < last_input[0].size(); j++){
                dout_dt[i][j] *= dLdZ[label_i][label_j];
            }
        }
